...
import { paper, resData } from './types';
function sleep(ms:number) {
  return new Promise(resolve => setTimeout(resolve, ms));
}
// Search async function
export async function searchInSemantic(query:string,maxResults=15,retries = 3, delay = 500):Promise<unknown[]>{

// Searching for the query
try{
const response = await axios.get(KEYS.SEMANTIC_CONFIG.api_url,  {params:{
  'query': `${query}`,
  'limit': maxResults,
},
  headers:{
