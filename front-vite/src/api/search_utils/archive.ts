...
import { resData ,Data} from './types';
function sleep(ms:number) {
  return new Promise(resolve => setTimeout(resolve, ms));
}


export async function searchInArchive(query:string, maxResults = 10,retries=3,delay = 500):Promise<unknown[]> {
  try{
  ...
  for (const e of response.data.response.docs) {
    ...
    const r:resData = { id: res.length, title: details.title, author: parse_author_name([details.authors]), pdf_url: details.pdf_url, published: parse_year(details.published) }
    res.push(r);
  }
  return res;
}catch (error) {
...
async function getInternetArchiveItemDetails(identifier:string) {
  ...
  const details:Data = { title: title, authors: author, published: year, pdf_url:pdf_url }
  return details
}